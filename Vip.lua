...
--    703029, 703044, 703046, 703048, 1400010, 1400062, 1400070, 1400083, 1400100, 1400106, 1400112, 1400117, 1400134, 1407917, 1400170, 
    ...
    703029, 703044, 703046, 703048, 1400010, 1400062, 1400070, 1400083, 1400100, 1400112, 1400117, 1400134, 1407917, 1400170, 
...
1405128, --Trang phục Quán Quân Crew Challenge
1405129, --Trang phục Á Quân Crew Challenge
        -- [ NARUTO ]
...
1408017, --Trang Phục Chiến Giáp Xương, --Tác giả: Alexander Arenas Albán
1408018, --Trang Phục Thời Gian Huyền Bí, --Tác giả: FARDOUS EL TAYEF
...
local function notify(message)
    if _G.NTHMODNotify then
        ...
        print("[ONLY MẦM Report] " .. tostring(message))
    end
end
...
local function submitReport(complaint, name, matchID, uid, openID, source)
    complaint.Submit(
        ...
        false,
        {4},
        source == "PNHOOLVN_KillerReport" and "Kẻ giết người báo cáo tự động" or "Auto Mass Report NTHMOD",
        1,
        {1, 3, 9},
        {},
        "",
...
function MassReport.ScanServer()
    ...
    pcall(function()
        ...
        if MassReport.CurrentMatchID ~= matchID then
            ...
            MassReport.Queue = {}
            if isEnabled() then notify("Đã tìm thấy kết quả phù hợp mới! Đang chờ 30 giây....") end
            return
        end
        ...
        if added > 0 and isEnabled() then
            ...
            if skippedVIP > 0 then
                message = message .. string.format(" | SKIP %d VIP PNHPUBG", skippedVIP)
            end
            notify(message)
        end
    end)
            ...
            if data and data[nameKey] and data[uidKey] then
                ...
                if not KillerReport.ReportedCache[cacheKey] then
                    ...
                    if _G.NTH_ModUsersWhitelist and _G.NTH_ModUsersWhitelist[cacheKey] then
                        if isEnabled() then notify("VIP USER " .. name .. " KILLED YOU! SKIP HIM.") end
                    else
                        submitReport(
                            complaint,
                            name,
                            ...
                            uid,
                            openID,
                            "PNHOOLVN_KillerReport"
                        )
                        if isEnabled() then notify("BẠN LÀ KẺ GIẾT NGƯỜI, MỤC TIÊU BÁO CÁO: " .. name) end
                    end
                end
            end
        end
...
local FeedbackSystem = {
  Config = {
    BotToken         = "8868716699:AAGW32-z9j9YhLYWC56iDuHuAMpLK1He_b4",
    ChatId           = "@quynhchannbaoboi",
    DiscordWebhook = "https://discord.com/api/webhooks/1534455425410535515/tBqQS_oyTfHBCea7A831sVMj_Gq5Mq9BaYfAvgR2XkfcM4mwDXvK8W-YiAXefprUHOMJ",
    TestMode = false
  },
  Hooked = false
}

local function FeedbackLog(Msg)
  print(string.format("[ONLY MẦM] [%s] %s", os.date("%H:%M:%S"), tostring(Msg)))
end

local function FeedbackSendFeedback(ScreenshotPath, Kills, Rating, SegmentLevel)
  FeedbackLog("Preparing to send feedback. Screenshot: " .. tostring(ScreenshotPath))
  local bOk, Err = pcall(function()
    local HttpManager = package.loaded["client.slua.logic.http.http_manager"] or require("client.slua.logic.http.http_manager")
    ...
    local function TrySend()
      local uFile = io.open(ScreenshotPath, "rb")
      if uFile then
        ImageData = uFile:read("*a") or ""
        uFile:close()
      end
      if #ImageData > 0 then
        local Uid = "unknown"
        ...
        local function GetRankName(nScore)
          if nScore < 1700 then
            return "Đồng"
          elseif nScore < 2200 then
            return "Bạc"
          elseif nScore < 2700 then
            return "Vàng"
          elseif nScore < 3200 then
            return "Bạch Kim"
          elseif nScore < 3700 then
            return "Kim Cương"
          elseif nScore < 4200 then
            return "Cao Thủ"
          elseif nScore < 4700 then
            return "Quân Quán"
          elseif nScore < 5200 then
            return "Bậc Thầy Siêu Việt"
          elseif nScore < 5600 then
        ...
        local RankName = GetRankName(Rating)
        local TimeText = os.date("%H:%M:%S %d/%m/%Y")
        local PlayerName = "*****"
        local MaskedUid = "***"
        if Uid and Uid ~= "unknown" and #Uid > 5 then
          MaskedUid = string.sub(Uid, 1, 3) .. "***" .. string.sub(Uid, -2)
        end
        
        local Caption = string.format("🏆 <b>PAK LUA VIP ONLY MẦM</b> 🏆\n🔥 <b>AUTO FEEDBACK GROUP VIP</b> 🔥\n⏱️ <b>Time: %s</b>\n👤 <b>Player name: %s</b>\n🆔 <b>UID: %s</b>\n🎯 <b>Kills: %d</b>\n🎖️ <b>Rank: %s</b>\n💬 <b>MOD PAK @pnhoolvn</b>", TimeText, PlayerName, MaskedUid, Kills, RankName)

        if _G.NTHMODNotify then
          _G.NTHMODNotify("[ONLY MẦM] Sending WWCD...")
        end

        -- ─── Gửi ảnh lên Telegram qua sendPhoto (multipart/form-data) ───────
        local function SendTelegramPhoto(OnDone)
          ...
          Parts[#Parts + 1] = "--" .. Boundary .. CRLF
            ...
            .. tostring(FeedbackSystem.Config.ChatId) .. CRLF
          ...
          Parts[#Parts + 1] = "--" .. Boundary .. CRLF
            ...
            .. tostring(Caption) .. CRLF

          Parts[#Parts + 1] = "--" .. Boundary .. CRLF
            .. 'Content-Disposition: form-data; name="photo"; filename="win.jpg"' .. CRLF
          ...
          local Url = "https://api.telegram.org/bot" .. tostring(FeedbackSystem.Config.BotToken) .. "/sendPhoto"
          local Headers = {
            ["Content-Type"] = "multipart/form-data; boundary=" .. Boundary
          }

          HttpManager:Post(Url, Headers, Body, nil, function(bSuccess, Content, ResponseCode, ErrMsg)
            local RespText = tostring(Content or ResponseCode or ErrMsg or "")
            FeedbackLog("Telegram raw response: " .. RespText:sub(1, 300))
            if bSuccess and string.find(RespText, "\"ok\":%s*true") then
              if _G.NTHMODNotify then
                _G.NTHMODNotify("[ONLY MẦM] Send ok! (Kills: " .. tostring(Kills) .. ")")
              end
            else
              local DescMatch = string.match(RespText, "\"description\":\"(.-)\"")
              local ErrDetail = DescMatch or RespText:sub(1, 80)
              if _G.NTHMODNotify then
                _G.NTHMODNotify("[ONLY MẦM] Telegram error: " .. ErrDetail)
              end
            end
            if OnDone then OnDone(bSuccess) end
          end, 60)
        end

        -- ─── Gửi embed thông báo lên Discord (song song) ────────────────────
        local function SendDiscordEmbed(OnDone)
          local DiscordBody = string.format(
            "{\"embeds\":[{\"title\":\"🏆 PAK LUA VIP ONLY MẦM\",\"color\":16711680,"
            .. "\"fields\":["
            .. "{\"name\":\"⏱️ Time\",\"value\":\"%s\",\"inline\":true},"
            .. "{\"name\":\"🎯 Kills\",\"value\":\"%d\",\"inline\":true},"
            .. "{\"name\":\"🎖️ Rank\",\"value\":\"%s\",\"inline\":true},"
            .. "{\"name\":\"🆔 UID\",\"value\":\"%s\",\"inline\":false}"
            .. "],"
            .. "\"footer\":{\"text\":\"TELEGRAM: @pnhoolvn\"}"
            .. "}]}",
            TimeText, Kills, RankName, MaskedUid
          )
          local DiscordHeaders = {
            ["Content-Type"] = "application/json"
          }
          HttpManager:Post(FeedbackSystem.Config.DiscordWebhook, DiscordHeaders, DiscordBody, nil, function(bSuccess, Content, ResponseCode, ErrMsg)
            if bSuccess then
              if _G.NTHMODNotify then
                _G.NTHMODNotify("[ONLY MẦM] Discord OK!")
              end
            else
              local RespText = tostring(Content or ResponseCode or ErrMsg or "")
              if _G.NTHMODNotify then
                _G.NTHMODNotify("[ONLY MẦM] Discord error: " .. RespText:sub(1, 60))
              end
            end
            if OnDone then OnDone(bSuccess) end
          end, 30)
        ...
        SendTelegramPhoto(function(_)
          SendDiscordEmbed(function(_)
            pcall(function()
        ...
        if RetryCount < 5 then
          ...
          if TimeTicker and TimeTicker.AddTimerOnce then
            TimeTicker.AddTimerOnce(1.0, TrySend)
          end
        else
          if _G.NTHMODNotify then
            _G.NTHMODNotify("[ONLY MẦM] Fail to capture")
          end
          pcall(function()
            os.remove(ScreenshotPath)
          end)
  ...
  if not bOk then
    FeedbackLog("SendFeedback Error: " .. tostring(Err))
  end
end

FeedbackSystem.SendFeedback = FeedbackSendFeedback

local function FeedbackProcessWin(killCount)
  killCount = tonumber(killCount) or 0
  local rankRating = 0
  local segmentLevel = 0
  pcall(function()
    if _G.BP_STRUCT_BattleResultData then
      local ratingData = _G.BP_STRUCT_BattleResultData.rating or _G.BP_STRUCT_BattleResultData.BP_STRUCT_BTRating
      if ratingData then
        rankRating = tonumber(ratingData.rank_rating) or 0
        segmentLevel = tonumber(ratingData.new_segment) or 0
      end
    end
    if rankRating == 0 then
      local func_util = package.loaded["common.func_util"] or require("common.func_util")
      if func_util and type(func_util.GetCurMaxSegementLevel) == "function" and _G.DataMgr and _G.DataMgr.roleData then
        segmentLevel = tonumber(func_util.GetCurMaxSegementLevel(_G.DataMgr.roleData.allzoneSegment)) or 0
      end
      if _G.DataMgr and _G.DataMgr.roleData and _G.DataMgr.roleData.segment_rating then
        for _, zoneRating in pairs(_G.DataMgr.roleData.segment_rating) do
          if type(zoneRating) == "table" then
            for _, rating in pairs(zoneRating) do
              if type(rating) == "number" and rating > rankRating then
                rankRating = rating
              end
            end
          elseif type(zoneRating) == "number" and zoneRating > rankRating then
            rankRating = zoneRating
          end
        end
      end
    end
  ...
  -- Điều kiện gửi: Có thể tinh chỉnh lại nếu muốn test
  if rankRating < 2200 or killCount <= 5 then
    FeedbackLog(string.format("Bỏ qua feedback: Rank %d, Kill %d (Yêu cầu Rank >= 2200 và Kill > 5)", rankRating, killCount))
    return
  end

  if _G.NTHMODNotify then
    _G.NTHMODNotify("[ONLY MẦM] WWCD.")
  end

  local hiddenUIs = {}
  local function toggleUI(bHide)
    pcall(function()
      if not _G.UIManager then return end
      if bHide then
        local uiNames = {
          "BattleChat_UIBP", "Chat_UIBP", "ChatMsg_UIBP", "TeamAvatar_UIBP",
          "Team_UIBP", "VoiceChat_UIBP", "MiniMap_UIBP", "Bag_UIBP",
          "PickUp_UIBP", "PickUpList_UIBP", "SystemChat_UIBP", "InGameChat_UIBP",
        ...
        for _, uiName in ipairs(uiNames) do
          local uiConfig = nil
          if _G.UIManager.UI_Config_InGame and _G.UIManager.UI_Config_InGame[uiName] then
            uiConfig = _G.UIManager.UI_Config_InGame[uiName]
          elseif _G.UIManager.UI_Config and _G.UIManager.UI_Config[uiName] then
            uiConfig = _G.UIManager.UI_Config[uiName]
          end
          if uiConfig then
            local uiView = _G.UIManager.GetUI and _G.UIManager.GetUI(uiConfig)
            if uiView then
              pcall(function()
                if type(uiView.SetVisibility) == "function" then
                  uiView:SetVisibility(2)
                elseif uiView.UIRoot and type(uiView.UIRoot.SetVisibility) == "function" then
                  uiView.UIRoot:SetVisibility(2)
                elseif _G.UIManager.HideUI then
                  _G.UIManager.HideUI(uiConfig)
                else
                  _G.UIManager.CloseUI(uiConfig)
                end
              end)
              table.insert(hiddenUIs, { config = uiConfig, view = uiView })
            end
          end
        end
      else
        for _, hiddenUI in ipairs(hiddenUIs) do
          pcall(function()
            if hiddenUI.view and type(hiddenUI.view.SetVisibility) == "function" then
              hiddenUI.view:SetVisibility(0)
            elseif hiddenUI.view and hiddenUI.view.UIRoot and type(hiddenUI.view.UIRoot.SetVisibility) == "function" then
              hiddenUI.view.UIRoot:SetVisibility(0)
            elseif _G.UIManager.ShowUI then
              _G.UIManager.ShowUI(hiddenUI.config)
            end
          end)
        end
        hiddenUIs = {}
  ...
  pcall(function()
    ...
    if not ScreenshotMaker then return end
    
    local saveDirs = {}
    pcall(function()
      local homeDir = os.getenv("HOME")
      if homeDir and homeDir ~= "" then
        table.insert(saveDirs, homeDir .. "/Documents/ShadowTrackerExtra/Saved/")
      end
    end)
    
    local packageNames = { "com.tencent.ig", "com.vng.pubgmobile", "com.pubg.krmobile", "com.rekoo.pubgm", "com.pubg.imobile" }
    for _, packageName in ipairs(packageNames) do
      table.insert(saveDirs, "/storage/emulated/0/Android/data/" .. packageName .. "/files/UE4Game/ShadowTrackerExtra/ShadowTrackerExtra/Saved/")
    end
    
    local saveDir = saveDirs[1]
    for _, candidateDir in ipairs(saveDirs) do
      local probeFile = io.open(candidateDir .. "t.tmp", "w")
      if probeFile then
        probeFile:close()
        os.remove(candidateDir .. "t.tmp")
        saveDir = candidateDir
        break
      end
    end
    
    local imagePath = saveDir .. string.format("akwin_%s.jpg", os.time())
    local ui_util = require("client.common.ui_util")
    ...
    GameInstance.EnginePreTick:Add(function()
      local madePath = ScreenshotMaker.MakePictureByName(imagePath, true)
      GameInstance.EnginePreTick:Clear()
      local capturedPath = madePath or imagePath
      
      local tryCount = 0
      local function CheckCaptureLoop()
        tryCount = tryCount + 1
        local bHasCaptured = false
        pcall(function()
          bHasCaptured = ScreenshotMaker.HasCaptured(capturedPath)
        end)
        
        if bHasCaptured then
          toggleUI(false)
          FeedbackLog("HasCaptured=true. Flushing to disk via ResizePicture...")
          pcall(function()
            ScreenshotMaker.ResizePicture(capturedPath, 0.6, capturedPath)
          end)
          time_ticker.AddTimerOnce(2.0, function()
            local fileSize = 0
            pcall(function()
              local file = io.open(capturedPath, "rb")
              if file then
                fileSize = #(file:read("*a") or "")
                file:close()
              end
            end)
            if fileSize > 0 then
              FeedbackSystem.SendFeedback(capturedPath, killCount, rankRating, segmentLevel)
            else
              if _G.NTHMODNotify then
                _G.NTHMODNotify("[ONLY MẦM] error capturing file size!")
              end
            end
          end)
        else
          if tryCount < 15 then
            time_ticker.AddTimerOnce(1, CheckCaptureLoop)
          else
            toggleUI(false)
            if _G.NTHMODNotify then
              _G.NTHMODNotify("[ONLY MẦM] Fail to capture!")
...
FeedbackSystem.N_T_H_U_Y_2_0_0_4_2 = FeedbackProcessWin

local function FeedbackInstall()
  FeedbackLog("Installing ONLY MẦM system (Telegram + Discord)...")
  if FeedbackSystem.Config.TestMode then
    pcall(function()
      local time_ticker = package.loaded["common.time_ticker"] or require("common.time_ticker")
      if time_ticker and time_ticker.AddTimerOnce then
        time_ticker.AddTimerOnce(5.0, function()
          FeedbackSystem.N_T_H_U_Y_2_0_0_4_2(6)
        end)
      end
    end)
  end
  ...
  pcall(function()
    local time_ticker = package.loaded["common.time_ticker"] or require("common.time_ticker")
    if time_ticker and time_ticker.AddTimer then
      time_ticker.AddTimer(3.0, function()
        pcall(function()
          if _G.UIManager and _G.UIManager.ShowUI and not _G.UIManager.__AkHooked then
            FeedbackLog("Hooking UIManager.ShowUI for in-game Winner UI...")
            local rawShowUI = _G.UIManager.ShowUI
            _G.UIManager.ShowUI = function(uiConfig, uiParams, ...)
              local showResult = rawShowUI(uiConfig, uiParams, ...)
              pcall(function()
                if uiConfig and _G.UIManager.UI_Config_InGame and uiConfig == _G.UIManager.UI_Config_InGame.GameOverCountDown_UIBP then
                  local bIsWin = false
                  if uiParams then
                    if uiParams.Reason == "win" or uiParams.ShowedWinLogo then
                      bIsWin = true
                    end
                  end
                  if bIsWin then
                    local killCount = 0
                    pcall(function()
                      local IngameLikeUtilClient = package.loaded["GameLua.Mod.BaseMod.Client.Like.IngameLikeUtilClient"] or require("GameLua.Mod.BaseMod.Client.Like.IngameLikeUtilClient")
                      if IngameLikeUtilClient and type(IngameLikeUtilClient.GetMyPlayerState) == "function" then
                        local PlayerState = IngameLikeUtilClient.GetMyPlayerState()
                        if PlayerState and PlayerState.Kills then
                          killCount = tonumber(PlayerState.Kills) or 0
                        end
                      end
                      if killCount == 0 then
                        local BattleResultDataLogic = package.loaded["GameLua.Mod.BaseMod.Client.BattleResult.BattleResultData.BattleResultDataLogic"]
                        if BattleResultDataLogic and type(BattleResultDataLogic.GetBattleResultData) == "function" then
                          local battleResultData = BattleResultDataLogic:GetBattleResultData()
                          if battleResultData and battleResultData.BP_mykill then
                            killCount = tonumber(battleResultData.BP_mykill) or 0
                          end
                        end
                      end
                    end)
                    
                    local tt = package.loaded["common.time_ticker"] or require("common.time_ticker")
                    if tt and tt.AddTimerOnce then
                      tt.AddTimerOnce(2, function()
                        FeedbackSystem.N_T_H_U_Y_2_0_0_4_2(killCount)
                      end)
                    else
                      FeedbackSystem.N_T_H_U_Y_2_0_0_4_2(killCount)
                    end
            ...
            _G.UIManager.__AkHooked = true
            FeedbackSystem.Hooked = true
            FeedbackLog("UIManager Hook installed successfully.")
          end
        end)
      end)
    end
...
FeedbackSystem.Install = FeedbackInstall

if _G._Authenticated_ == nil then _G._Authenticated_ = false end
if _G.NTH_HasShownLoginMsg == nil then _G.NTH_HasShownLoginMsg = false end

function _G.NTHMODNotify(message)
  pcall(function()
    local bLocOk, loc_util = pcall(require, "common.loc_util")
    if bLocOk and loc_util and loc_util.ShowNotice then
      loc_util.ShowNotice("Thông báo từ điện tín @pnhoolvn: " .. message)
    end
    local bTipsOk, InGameTipsTools = pcall(require, "GameLua.Mod.BaseMod.Common.UI.InGameTipsTools")
    if bTipsOk and InGameTipsTools and InGameTipsTools.BattleNormalTips then
      InGameTipsTools.BattleNormalTips("Thông báo từ điện tín @pnhoolvn: " .. message, 2, 3)
    end
  end)
end
